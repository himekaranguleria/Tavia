 ...
 const MODULE_NAME = 'Modulebazaar_Firstdataapi';
    ...
    */
    ...
    public function createMageOrders($subscription) {
		...
		$quote->getShippingAddress()->setShippingMethod('freeshipping_freeshipping')->save();	
...
//          $process = false;
            if ($process) {
               ...
               // $this->ordernotifier->notify($order);
				...
				$order = $orderModel->load($order_id);
                $order->getPayment()->setMethod('firstdataapi');
                $msg         = 'Transaction status: '.$result['transaction_status'].' | Bank Message: '.$result['bank_message'].' | Gateway Message: '.$result['gateway_message'];
                if ($order->getId()) {
